...
def parse_score(match_score: str, match_id: str, tourney_id:str) -> Array:
    ...
    return([match_id, match_score, match_ret, winner_sets_won, winner_games_won, winner_tiebreaks_won, loser_sets_won, loser_games_won, loser_tiebreaks_won])

# main
# logging support
#logzero.logfile('update_scores.log', loglevel=logzero.logging.WARNING)
logzero.logfile('update_scores.log', loglevel=logzero.logging.INFO)
...
#    sql = 'select v.match_id, v.tournament_code, v.match_score_raw from vw_match_scores v where v.tournament_year = 2019'
    ...
    for match in matches:
        if adjust_score(match[0], match[2]) != match[2]:
            matches_data.append(parse_score(adjust_score(match[0], match[2]), match[0], str(match[1])))
    ...
    # 1. insert into stg
    cur.executemany('insert into stg_match_scores(match_id, match_score, match_ret, winner_sets_won, winner_games_won, winner_tiebreaks_won, loser_sets_won, loser_games_won, loser_tiebreaks_won) values (:1, :2, :3, :4, :5, :6, :7, :8, :9)',
                    matches_data)
    con.commit()
    # 2. process
    logzero.logger.info('start data processing')
    ...
    # store in CSV
    csv_file = open('update_scores.csv', 'w', encoding='utf-8')
