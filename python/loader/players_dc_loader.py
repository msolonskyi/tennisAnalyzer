...
class PlayersDCLoader(BaseLoader):
    ...
    def _init(self):
        ...
        self.INSERT_STR = 'insert into stg_players(player_code, first_name, last_name, player_url, flag_code, birthdate) values (:1, :2, :3, :4, :5, :6)'
        self.PROCESS_PROC_NAME = 'sp_process_dc_players'
        super()._init()
    ...
    def _parse_player(self, url: str):
        try:
            ...
            player_birthdate = player.get('BirthDate')  # "18 February 1995"
            player_first_name = player.get('GivenName')
            player_last_name = str(player.get('FamilyName')).capitalize()
