...
try:
    ...
    for key, sql in EXTRACTION_LIST_BY_YEAR.items():
        for year in range(1999, datetime.today().year + 1):
            cur.execute(sql, {'year': year})
            col_names = [row[0].lower() for row in cur.description]
        
            csv_file = open(f'{CSV_PATH}{key}_{year}.csv', 'w', encoding='utf-8')
            writer = csv.writer(csv_file)
            writer.writerow(col_names)
            for row in cur:
                writer.writerow(row)
        
            logzero.logger.info(f'{key}: year {year}, {cur.rowcount} rows has been extracted')
            csv_file.close()
