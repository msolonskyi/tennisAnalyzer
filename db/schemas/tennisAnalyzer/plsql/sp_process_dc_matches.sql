...
begin
  ...
  using(select i.id,
        ...
        from (select sm.tournament_id || '-' || w.code || '-' || l.code || '-' || sm.stadie_id as id,
                     sm.tournament_id,
                     sm.stadie_id,
                     sm.match_order,
                     sm.match_ret,
              ...
              from stg_matches sm, players w, players l, matches m
              where sm.winner_code = w.code_dc(+)
                and sm.loser_code = l.code_dc(+)
                and m.id(+) = sm.tournament_id || '-' || w.code || '-' || l.code || '-' || sm.stadie_id) i) s
  on (s.id = d.id)
