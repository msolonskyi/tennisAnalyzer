create or replace function sf_atp_draws_delta_hash(
  pv_id                  draws.id%type,
  pv_left_match_draw_id  draws.left_match_draw_id%type,
  pv_right_match_draw_id draws.right_match_draw_id%type,
  ...
  pv_stadie_id           draws.stadie_id%type,
...
begin
  select ora_hash(pv_match_id || '|' || pv_tournament_id || '|' || pv_left_player_code || '|' || pv_right_player_code || '|' || pv_stadie_id || '|' || pv_id || '|' || pv_left_match_draw_id || '|' || pv_right_match_draw_id)
  into vn_delta_hash
  from dual;
