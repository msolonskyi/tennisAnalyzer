create or replace procedure sp_upd_dc_matches_delta_hash(
  pv_id dc_matches.id%type)
is
begin
  update dc_matches m
    set
      delta_hash = sf_dc_matches_delta_hash(
             pv_id                         => m.id,
             pv_tournament_id              => m.tournament_id,
             pv_stadie_id                  => m.stadie_id,
             pn_match_order                => m.match_order,
             pv_match_ret                  => m.match_ret,
             pn_winner_id                  => m.winner_id,
             pn_loser_id                   => m.loser_id,
  ...
  where m.id = pv_id;
end sp_upd_dc_matches_delta_hash;
/
